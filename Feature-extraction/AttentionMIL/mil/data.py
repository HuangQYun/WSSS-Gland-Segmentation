...
from FeaturesbasedMIL.data import EncodedDataset, MapDataset, SKLearnEncoder
...
def get_deploy_cohort_df(feature_dir: Union[Path, str],target_label: str) -> pd.DataFrame:
    ...
    h5_df[target_label] = 'nonMSIH'
    len = h5_df.shape[0]
    if len > 1:
        h5_df[target_label][len-1] = 'MSIH'
        df = h5_df
    elif len==1:
